...
void check(T result, char const *const func, const char *const file,
    int const line) {
        if (result) {
            fprintf(stderr, "CUDA error at %s:%d, code=%s(%d) \"%s\"", file, line, 
            static_cast<unsigned int>(result), _cudaGetErrorNum(result), func);
            exit(EXIT_FAILURE);
        }
    }
...
inline const char* _ConvertSMVer2ArchName(int major, int minor) {
    ...
    printf("MapSMtoArchName for SM %d.%d is undefined."
        " Default to use %s\n",
        major, minor, nGpuArchNameSM[index - 1].name); // Graphics Device
...
inline int GpuDeviceInit(int device_id) {
    ...
    if (device_id > device_cnt - 1) {
        ...
        fprintf(stderr, ">> GpuDeviceInit (-device=%d) is not a valid"
            " GPU device. <<\n");
        fprintf(stderr, "\n");
        return -device_id;
    }
    ...
    printf("GPUDeviceInit CUDA Device [%d]: \"%s\"\n", device_id, _ConvertSMVer2ArchName(major, minor));
...
inline int GpuGetMaxGflopsDeviceId() {
    ...
    while (current_device < device_count) {
        ...
        int clock_rate = 0;
        checkCudaErrors(cudaDeviceGetAttribute(&multi_process_cnt, cudaDevAttrMultiProcessorCount, current_device));
        ...
        if (result != cudaSuccess) {
            ...
            if (result == cudaErrorInvalidValue) {
                ...
                static_cast<unsigned int>(result), _cudaGetErrorNum(result));
                exit(EXIT_FAILURE);
            }
        }
